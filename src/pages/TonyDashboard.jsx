...
export default function TonyDashboard({ onLogout }) {
  ...
  // dedup : si un client a un [VERSEMENT] ce mois-ci, on ne compte pas sa session en double
  ...
  const caJour  = dedup(sessJ).reduce((a,s)=>a+(s.properties.Prix?.number||0),0) + versJ.reduce((a,s)=>a+(s.properties.Prix?.number||0),0)
  ...
  return (
    <div style={{background:'var(--bg)',minHeight:'100dvh',paddingBottom:'88px'}}>
      ...
      {versJ.length > 0 && (
        <div style={{position:'sticky',top:0,zIndex:20,background:'linear-gradient(135deg,#1A8C5A,#16744A)',boxShadow:'0 2px 12px rgba(26,140,90,.35)'}}>
          {versJ.map((v, i) => {
            ...
            return (
              <div key={v.id} style={{display:'flex',alignItems:'center',gap:'10px',padding:'10px 16px',borderBottom: i < versJ.length-1 ? '1px solid rgba(255,255,255,.1)' : 'none'}}>
                <span style={{fontSize:'18px'}}>💰</span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:'13px',fontWeight:700,color:'#fff',lineHeight:1.2}}>Acompte reçu — {client} · {montant}€</div>
                  {(rdvDate || style) && (
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'20px 20px 14px',background:'var(--surface)',borderBottom:'1px solid var(--border)',position:'sticky',top: versJ.length > 0 ? (versJ.length * 52) + 'px' : '0',zIndex:10,boxShadow:'0 2px 8px rgba(26,18,9,.04)'}}>
      ...
      <div style={{padding:'16px 16px 0'}}>
                          ...
                          <div style={{flex:1,padding:'11px 12px',display:'flex',justifyContent:'space-between',alignItems:'center',minWidth:0}}>
                            ...
                            <div style={{flexShrink:0,marginLeft:'10px',textAlign:'right'}}>
                              ...
                              <div style={{display:'flex',gap:'5px',marginTop:'5px',justifyContent:'flex-end'}}>
                                <button onClick={()=>{const dr=s.properties.Date?.date?.start||'';const h=dr.includes('T')?dr.substring(11,16):'';setEditRdv({id:s.id,client:s.properties['Client prénom']?.rich_text?.[0]?.plain_text||'',style:s.properties['Style / Type']?.rich_text?.[0]?.plain_text||'',prixEstime:String(s.properties.Prix?.number||0),sessions:String(getNbSess(s)),acompte:String(s.properties['Acompte reçu']?.number||0),date:dr.split('T')[0]||date,heure:h,natio:s.properties.Nationalité?.select?.name||'🇫🇷 FR',source:s.properties.Source?.select?.name||'📸 Instagram',existingNotes:s.properties.Notes?.rich_text?.[0]?.plain_text||''});setTab('editRdv')}} style={{padding:'5px 8px',borderRadius:'8px',fontSize:'13px',cursor:'pointer',background:'var(--bg2)',border:'1px solid var(--border2)',color:'var(--txt3)'}}>✏️</button>
                              </div>
                            </div>
                          </div>
                        </div>
        ...
        {(()=>{
          ...
          const joursDimanche  = todayDow === 0 ? 0 : 7 - todayDow // jours jusqu'au prochain dimanche
          const joursFinMois   = daysInM - dayOfM // jours restants après aujourd'hui
