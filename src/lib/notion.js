...
export const notion = {
  // Sessions & CA
  getSessions: (startCursor) => api(`databases/${SESSIONS_DB}/query`, 'POST', {
    sorts: [{ property: 'Date', direction: 'descending' }],
    page_size: 50,
    ...(startCursor ? { start_cursor: startCursor } : {})
  }),

  addSession: (data) => api('pages', 'POST', {
    ...
    properties: {
      ...
      'Avis Google': { checkbox: data.avis || false },
      Notes: { rich_text: [{ text: { content: data.notes || '' } }] },
      Date: { date: { start: data.date } }
    }
  }),

  updateSession: (pageId, data) => api(`pages/${pageId}`, 'PATCH', {
    properties: {
  ...
  // Clients CRM
  getClients: () => api(`databases/${CLIENTS_DB}/query`, 'POST', {
    sorts: [{ timestamp: 'created_time', direction: 'descending' }],
    page_size: 100
  }),

  addClient: (data) => api('pages', 'POST', {
  ...
  updateClientStatus: (pageId, statut) => api(`pages/${pageId}`, 'PATCH', {
    properties: { Statut: { select: { name: statut } } }
  }),

  // KPIs
  addKPI: (data) => api('pages', 'POST', {
