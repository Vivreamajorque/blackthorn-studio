  ...
  return (
    <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--noir2)', borderTop: '1px solid var(--noir3)', display: 'flex', justifyContent: 'space-around', padding: '8px 0 calc(8px + env(safe-area-inset-bottom))', zIndex: 100 }}>
      {NAV.map(({ path, icon, label }) => {
        ...
        return (
          <button key={path} onClick={() => navigate(path)} style={{ background: 'none', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', color: active ? 'var(--pierre)' : 'var(--gris2)', padding: '4px 6px', transition: 'color .2s', minWidth: 44 }}>
            <span style={{ fontSize: '18px', lineHeight: 1 }}>{icon}</span>
            <span style={{ fontSize: '8px', fontFamily: 'var(--font-head)', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>{label}</span>
          </button>
        )
      })}
    </nav>
